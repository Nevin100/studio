...
const WhatWeDo = ({ isBad = false }: { isBad?: boolean }) => {
  ...
  if (isBad) {
    return (
      <div>
        ...
        >
          From hackathon sites to department portals, we build it all.
        </p>
        <div style={{ display: "flex", marginTop: "2rem", gap: "1rem" }}>
          <div style={{ flex: 1, border: "3px solid green", padding: "1rem" }}>
            <h3 style={{ fontSize: "1.8rem", color: "yellow" }}>
              achievements.log
            </h3>
            <ul>
              {achievements.map((item, i) => (
                <li key={i} style={{ marginLeft: `${i * 10}px` }}>
                  {item}
                </li>
              ))}
            </ul>
          ...
          <div style={{ flex: 1, border: "1px dotted magenta", padding: "2rem" }}>
            <h3 style={{ fontSize: "1.2rem", color: "cyan" }}>
              cross_department.sh
            </h3>
            <div>
              {departments.map((item, i) => (
                <p key={i} style={{ paddingLeft: `${i * 5}px` }}>
                  {item}
                </p>
              ))}
            </div>
  ...
  return (
    <section id="what-we-do" className="container mx-auto px-4">
                ...
                >
                  {achievements.map((item, index) => (
                    <motion.li
                      ...
                      className="log-item flex items-start gap-3"
                    >
            ...
            <div className="p-8">
              ...
              <CardContent className="p-0">
                <motion.div
                  ...
                  whileInView="visible"
                  viewport={{ once: true, amount: 0.4 }}
                  transition={{ delayChildren: 0.3 }} // mimic GSAP "-=0.3" overlap
                  className="space-y-3 font-code text-lg"
                >
