  ...
  if (isBad) {
    return (
      <div>
        ...
        <h2 style={{ fontSize: "1.5rem", color: "fuchsia" }}>
          Building the Web, Building the Future
        </h2>
        <motion.p
          className="text-lg md:text-xl text-foreground mb-8 max-w-3xl mx-auto text-center"
          initial={{ y: 30, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          transition={{ type: "spring", stiffness: 120, delay: 0.3 }}
        >
          We are the architects of the digital frontier, a community of
          developers, designers, and innovators at GeekRoom dedicated to
          building the web&apos;s future, one line of code at a time.
        </motion.p>
        ...
        >
          {terminalLines.map((line, index) => (
            <p key={index} className="terminal-line text-muted-foreground">
              <span
                className={
                  line.startsWith("$") ? "text-green-400" : "text-accent"
                }
              >
                {line.split(" ")[0]}
              </span>{" "}
          ...
          <div className="h-6">
            <span className="cursor text-primary text-2xl">▋</span>
          </div>
        </motion.div>
